...
class ReportAssistant:
    ...
    def load_yesterday_plans(self):
        ...
        for plan in new_plans:
            # Используем add_task для правильной нумерации
            self.task_entry.delete(0, "end")
            self.task_entry.insert(0, plan)
            self.add_task()
    ...
    def add_task(self):
        ...
        if task:
            ...
            # Добавляем с номером
            self.tasks_list.insert("end", f"{next_number}. {task}\n")
            self.task_entry.delete(0, "end")
    ...
    def complete_selected(self):
        ...
        try:
            ...
            if selected_text:
                tasks = [line.strip() for line in selected_text.split('\n') if line.strip()]
                for task in tasks:
                    if task:
                        # Убираем номер из задачи перед сохранением
                        if '. ' in task:
                            task_text = task.split('. ', 1)[1]
                        else:
                            task_text = task.lstrip('•- 1234567890.')
                        self.manager.add_completed_task(task_text)
    ...
    def renumber_tasks(self):
        """Перенумеровать все задачи"""
        # Получаем текущий текст
        current_text = self.tasks_list.get("1.0", "end-1c").strip()
        if not current_text:
            return
        ...
        # Добавляем заново с правильной нумерацией
        for i, line in enumerate(lines, 1):
            # Убираем старый номер (все что до первой точки)
            if '. ' in line:
                task_text = line.split('. ', 1)[1]
            else:
                task_text = line.lstrip('•- 1234567890.')
        
            self.tasks_list.insert("end", f"{i}. {task_text}\n")